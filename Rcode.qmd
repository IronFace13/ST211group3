...
## `homework vs k4score
ggplot(data, aes(x=homework,y=ks4score)) + geom_boxplot() + labs(title = 'Boxplot between Homework Ks4score')
```
Students spend more time on homework have a higher overall ks4score

## `sen vs k4score
```{r}
ggplot(data, aes(x=sen,y=ks4score)) + geom_boxplot() + labs(title = 'Boxplot between sen and Ks4score')
```
Students without SEN have a significantly higher ks4score compared to students with SEN
